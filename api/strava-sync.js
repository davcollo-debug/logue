  ...
  try {
    ...
    for (const activity of activities) {
      if (activity.type === 'Run') {
        const pace = activity.average_speed ? formatPace(activity.average_speed) : '0:00';
        const activityData = {
    ...
    return res.status(200).json({ activities: savedActivities, count: savedActivities.length });
